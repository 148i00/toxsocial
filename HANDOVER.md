...
ToxSocial 已完成 M0–M4 和大部分 M5 功能；桌面端已能打包发布，最新版本 **v0.2.19**。最近重点做了两件事：
...
### 待办功能
- 评论回复嵌套（当前只预填 `@用户名`，没有真正树形嵌套）。
- 用户资料页显示 bio（后端/协议已有 bio 字段，前端 profile 页还没展示）。
- 文件发送 UI（后端 `send_file_to_friend` 已实现，前端没有入口）。
